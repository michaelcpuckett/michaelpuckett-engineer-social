...
function Entity({ headingLevel, entity, user }) {
    const typedEntity = (0, activitypub_core_utilities_1.getTypedEntity)(entity);
    const entityType = typedEntity.type;
    for (const type of Object.values(activitypub_core_types_1.AP.ActivityTypes)) {
        if (entityType === type || (Array.isArray(entityType) &&
            entityType.includes(type))) {
            return react_1.default.createElement(ActivityEntity, { headingLevel: headingLevel, activity: typedEntity, user: user });
        }
    }
    for (const type of Object.values(activitypub_core_types_1.AP.ActorTypes)) {
        if (entityType === type || (Array.isArray(entityType) &&
            entityType.includes(type))) {
            return react_1.default.createElement(ActorEntity, { headingLevel: headingLevel, actor: typedEntity, user: user });
        }
    }
    if (entityType === activitypub_core_types_1.AP.CollectionTypes.COLLECTION || (Array.isArray(entityType) && entityType.includes(activitypub_core_types_1.AP.CollectionTypes.COLLECTION))) {
        return react_1.default.createElement(CollectionEntity, { headingLevel: headingLevel, collection: typedEntity, user: user });
    }
    if (entityType === activitypub_core_types_1.AP.CollectionTypes.ORDERED_COLLECTION || (Array.isArray(entityType) && entityType.includes(activitypub_core_types_1.AP.CollectionTypes.ORDERED_COLLECTION))) {
        return react_1.default.createElement(OrderedCollectionEntity, { headingLevel: headingLevel, collection: typedEntity, user: user });
    }
    if (entityType === activitypub_core_types_1.AP.ExtendedObjectTypes.NOTE || (Array.isArray(entityType) && entityType.includes(activitypub_core_types_1.AP.ExtendedObjectTypes.NOTE))) {
        return react_1.default.createElement(NoteEntity, { headingLevel: headingLevel, note: typedEntity, user: user });
    }
    for (const type of Object.values(activitypub_core_types_1.AP.ExtendedObjectTypes)) {
        if (entityType === type || (Array.isArray(entityType) && entityType.includes(type))) {
            return react_1.default.createElement(ExtendedObjectEntity, { headingLevel: headingLevel, extendedObject: typedEntity, user: user });
        }
    }
    return react_1.default.createElement(react_1.default.Fragment, null);
}
